...
const mockTips = [
  {
    ...
    public_bias: "high",
    reasoning_text: "Arsenal have won 8 of their last 10 home games. Chelsea's away form has collapsed — 2 wins in 9. Key: Arsenal's press disrupts Chelsea's build-up play, and with Saka fully fit, the wide channels will be exploited. Value exists at 1.85 given true probability sits closer to 60%.",
    analyst_note: "Monitor lineup 2hrs before kickoff. Injury to Saliba could shift this.",
  ...
  },
  {
    ...
    public_bias: "medium",
    reasoning_text: "These sides have averaged 3.4 goals in their last 8 meetings. Both teams press high and defend with a high line — transitions are rapid and chaotic.",
    analyst_note: "Both managers have full squads. Weather: clear. Likely open game from minute 1.",
    match_date: "2026-03-09T17:30:00Z",
    is_premium: true,
    result_status: "pending",
  },
  {
    ...
    public_bias: "low",
    reasoning_text: "Lakers have covered the spread in 6 of last 8 home games vs West opponents. Warriors are 3-7 ATS on the road this season.",
    analyst_note: "Steph Curry listed as questionable. If he plays, revise confidence to 55%.",
    match_date: "2026-03-07T02:00:00Z",
    is_premium: true,
    result_status: "won",
  },
  {
    ...
    public_bias: "high",
    reasoning_text: "El Clasico has seen BTTS in 14 of the last 17 editions. Both keepers face elite forwards. Neither team has kept a clean sheet in their last 4 league games.",
    analyst_note: "High-pressure match, both teams will push forward from the start.",
  ...
  },
  {
    ...
    public_bias: "low",
    reasoning_text: "Celtics rank 3rd in defensive efficiency this season. Heat play slow, half-court offense. Pace stats suggest this game goes under the total 70% of the time.",
    analyst_note: "Back-to-back for Heat. Fatigue factor could suppress scoring further.",
    match_date: "2026-03-06T00:30:00Z",
    is_premium: false,
    result_status: "lost",
  },
];
...
const mockStats = {
  ...
  roi_percentage: 21.4,
  avg_odds: 1.82,
  football_win_rate: 71.2,
  basketball_win_rate: 64.8,
  last_10: ["won","won","lost","won","won","won","lost","won","won","lost"],
};

const mockSubRequests = [
  { id:"sub_001", user_id:"usr_002", email:"sarah@email.com", plan_type:"monthly", price:45000, payment_status:"pending", mobile_number:"256774123456", transaction_ref:"MTN987654321", created_at:"2026-03-04T10:30:00Z" },
  { id:"sub_002", user_id:"usr_003", email:"james@email.com", plan_type:"weekly", price:15000, payment_status:"pending", mobile_number:"256752987654", transaction_ref:"AIR123456789", created_at:"2026-03-05T08:15:00Z" },
];
...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .star { width: 16px; height: 16px; border-radius: 3px; transition: background 0.2s; }
  ...
  .btn-primary {
    background: var(--grad1); color: #000;
  ...
  .tips-table .table-row { grid-template-columns: 1fr 100px 90px 80px 90px 100px; }
  ...
  .result-btn {
    ...
    border: 1px solid; font-family: 'DM Sans', sans-serif; transition: all 0.15s;
  }
  ...
  .auth-tab {
    ...
    cursor: pointer; font-size: 14px; font-weight: 600; color: #6b7590;
  ...
  .loading-screen {
    ...
    color: #00e5a0; font-family: 'Syne', sans-serif;
...
function ValueStars({ rating }) {
  return (
    <div className="value-stars">
      {[1,2,3,4,5].map(i => (
        <div key={i} className={`star ${i <= rating ? "filled" : "empty"}`} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="bias-indicator">
      <div className="bias-bar">
        {[1,2,3].map(i => (
          <div key={i} className={`bias-seg ${i <= filled ? `filled ${bias}` : "empty"}`} />
        ))}
      </div>
    ...
    <div className={`tip-card ${isLocked ? "premium-locked" : ""} slide-in`}>
      ...
      <div className="tip-prediction-row">
        ...
        <div className="odds-pill">
          ...
          <span className="odds-value">{isLocked ? "—" : tip.odds_reference.toFixed(2)}</span>
        </div>
      </div>
      ...
      {!isLocked && (
        <div className="metrics-row">
          ...
          <div className="metric-item">
            ...
            <BiasIndicator bias={tip.public_bias} />
          </div>
        </div>
      )}
      ...
      {expanded && !isLocked && (
        <div className="reasoning-panel fade-in">
          ...
          {tip.analyst_note && (
            <div className="analyst-note">💡 <strong>Note:</strong> {tip.analyst_note}</div>
          )}
        </div>
      )}
    ...
    <div className="sub-page">
      ...
      <div className="plans-grid">
        {Object.entries(plans).map(([key, plan]) => (
          <div
            key={key}
            className={`plan-card ${selected === key ? "selected" : ""} ${key === "monthly" ? "popular" : ""}`}
            onClick={() => setSelected(key)}
            style={{marginTop: key === "monthly" ? 16 : 0}}
          >
            ...
            <ul className="plan-features">
              {[
                [true, "All premium tips daily"],
                [true, "Full reasoning breakdown"],
                [true, "Confidence % & value rating"],
                [true, "Performance analytics"],
                [key === "monthly", "Priority support"],
                [key === "monthly", "Early access to tips"],
              ].map(([included, text], i) => (
      ...
      <div className="payment-instruction">
        ...
        <p>
          ...
          2. Send <code>UGX {plans[selected].price.toLocaleString()}</code> to number <code>0754 123 456</code><br/>
...
function CreateTipModal({ onClose, onSave, notify, userId }) {
  ...
  const handleSave = async () => {
    ...
    const newTip = {
      ...
      confidence_percentage: form.confidence_percentage,
      public_bias: form.public_bias,
      reasoning_text: form.reasoning_text,
    ...
    if (error) {
      notify("Error saving tip: " + error.message, "error");
      return;
    }
      ...
      <div className="modal fade-in">
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <input className="form-input" placeholder="e.g. Arsenal Win / Over 2.5" value={form.prediction} onChange={e=>set("prediction",e.target.value)} />
          </div>
          <div className="form-group">
        ...
        <div className="form-group">
          ...
          <textarea className="form-textarea" placeholder="Provide data-backed reasoning for this prediction..." value={form.reasoning_text} onChange={e=>set("reasoning_text",e.target.value)} />
        </div>
        ...
        <div style={{display:"flex",gap:12,marginTop:24}}>
          ...
          <button className="btn-primary" onClick={handleSave} style={{flex:2,justifyContent:"center"}}>💾 Save Tip</button>
        </div>
      </div>
    </div>
  );
...
function AdminPage({ tips, setTips, notify, userId, allUsers }) {
  ...
  const [subs, setSubs] = useState(mockSubRequests);
  ...
  const updateResult = async (tipId, status) => {
    await supabase.from('tips').update({ result_status: status }).eq('id', tipId);
          ...
          <div className="admin-table">
            ...
            {subs.map(sub => (
              <div key={sub.id} className="table-row" style={{gridTemplateColumns:"1fr 100px 130px 130px 100px"}}>
                <div className="table-cell">
                  <div style={{fontWeight:600,fontSize:13}}>{sub.users?.email || sub.email}</div>
...
export default function BetSenseApp() {
  ...
  const [stats, setStats] = useState({ total_tips:0, won_tips:0, lost_tips:0, win_rate:0, roi_percentage:0, avg_odds:0, football_win_rate:0, basketball_win_rate:0, last_10:[] });
    ...
    <>
      ...
      <div className="app">
        ...
        <main className="main">
          ...
          {page === "subscription" && (
            <SubscriptionPage user={user} onSuccess={() => setPage("dashboard")} />
          )}
          {page === "admin" && isAdmin && (
